"""Section-based hierarchical text chunking."""
...
SECTION_SEPARATORS = [
    ...
    r'\n(?=\([a-z]\)\s)',               # "(a) the company..." — sub-clauses
...
def split_by_structure(text: str) -> list[str]:
    """
    Split text into sections using all regex separators combined.
    ...
    combined_pattern = "|".join(f"(?:{sep})" for sep in SECTION_SEPARATORS)
...
def abbreviate_source(source: str) -> str:
    """Derive a short, deterministic abbreviation from a source document name."""
    alnum = re.sub(r'[^A-Za-z0-9]', '', source)
    return (alnum[:6] or "SRC").upper()
...
def create_table_chunks(
    ...
    chunk_id format: SRC_T{n}  e.g. HANDBO_T1
    """
    table_chunks = []
    for idx, table in enumerate(tables, 1):
        chunk_id = f"{source_abbr}_T{idx}"
...
def create_chunks(
    ...
    """
    Create hierarchical chunks from parsed text.

    Process:
    1. Split by document structure (sections, sub-sections, sub-clauses)
    ...
    source_abbr = abbreviate_source(source)

    # Split into sections using all separators in a single combined pass
    section_texts = split_by_structure(text)
    ...
    for section_text in section_texts:
        ...
        parent_chunk_id = f"{source_abbr}_S{section_number}"
